...
void ThresholdOperator(GlobalGrid & Grid, double eps, double a, unsigned int & anzahl_gesamt, unsigned int & anzahl_klein) {
  const int levels = Grid.size();
...
double err_thr(GlobalGrid & Grid, double eps, double a) {
  const int levels = Grid.size();
  double sum = 0;
  for (int l = levels-2; l>=0; --l) {
    eps /= a;
...
double max_eps(GlobalGrid & Grid, double a) {
  const int levels = Grid.size();
...
double eps_opt(GlobalGrid & Grid, double err, double a) {
  double delta = pow(10.,floor(log10(err))-1);
  double eps_l = 0.;
  double eps_r = max_eps(Grid, a);
  double err_l = 0.;
  double err_r = err_thr(Grid, eps_r, a);
  if (err_r < err) return eps_r;
  while ((eps_r - eps_l) >= delta) {
    double eps_m = (eps_l+eps_r)/2.;
    double err_m = err_thr(Grid, eps_m, a);
    if (err_m == err) return eps_m;
    if (err_m < err) { eps_l = eps_m; err_l = err_m; }
    else { eps_r = eps_m; err_r = err_m; }
  }
  return eps_r;
}

void MultiScaleOperator(GlobalGrid & Grid){
  const int levels = Grid.size();
  for (int l = levels-2; l>=0; --l) {
    for (GridMap::iterator it = (Grid[l]).begin(); it != (Grid[l]).end(); ++it){
...
void InverseMultiScaleOperator(GlobalGrid & Grid){
  const int levels = Grid.size();
...
void ProjektionSinglescale(GlobalGrid & Grid, func F, int rule) {
  const int levels = Grid.size();
  int order_num;
  double *wtab;
  double *xytab;
  ...
  for (GridMap::iterator it = (Grid[levels-1]).begin(); it != (Grid[levels-1]).end(); ++it) {
    (it->second).u_coeff.resize(M);
    double eckpunkte[6] = {
      (it->second.nodes)[0].x, (it->second.nodes)[0].y,
      (it->second.nodes)[1].x, (it->second.nodes)[1].y,
      (it->second.nodes)[2].x, (it->second.nodes)[2].y };
    ...
    for (int i = 0; i < M; ++i) {
      ...
      for (int order = 0; order < order_num; ++order) {
        ...
        quad += wtab[order] * F(x,y) * sqrt(1./(2.*it->second.flaeche)) * skalierungsfunktion(i,tau(0),tau(1));
      }
...
// nicht sinnvoll
void ProjektionMultiscale(GlobalGrid & Grid, func F, int rule) {
  const int levels = Grid.size();
  int order_num;
  double *wtab;
  double *xytab;
    ...
    for (GridMap::iterator it = (Grid[l]).begin(); it != (Grid[l]).end(); ++it) {
      ...
      for (int i = 0; i < 3*M; ++i) {
        ...
        for (int order = 0; order < order_num; ++order) {
          ...
          vec tau(3); tau(0) = x; tau(1) = y; tau(2) = 1.;
          lr_solve(it->second.A, it->second.r, tau);
          quad += wtab[order] * F(x,y) * sqrt(1./(2.*it->second.flaeche)) * muttermultiwavelet(p,i,tau(0),tau(1));
        }
        quad *= it->second.flaeche;
...
double AuswertungSinglescale(GlobalGrid & Grid, double x, double y, int level, Key index) {
  vec tau(3); tau(0) = x; tau(1) = y; tau(2) = 1.;
  ...
  double sum = 0.;
  for (int i = 0; i < M; ++i) {
    sum += Grid[level][index].u_coeff[i] * sqrt(1./(2.*Grid[level][index].flaeche)) * skalierungsfunktion(i,tau(0),tau(1));
  }
  return sum;
}

double AuswertungMultiscale(GlobalGrid & Grid, double x, double y, int level, Key index) {
  vec tau(3); tau(0) = x; tau(1) = y; tau(2) = 1.;
  lr_solve(Grid[level][index].A, Grid[level][index].r, tau);
  // assert((tau(0)>=0.) && (tau(1)>=0.) && (tau(0)+tau(1)<=1.));
  double sum = 0.;
  while (Grid[level][index].hasFather) {
    ...
    lr_solve(Grid[level][index].A, Grid[level][index].r, tau);
    for (int i = 0; i < 3*M; ++i) {
      sum += Grid[level][index].d_coeff[i] * sqrt(1./(2.*Grid[level][index].flaeche)) * muttermultiwavelet(p,i,tau(0),tau(1));
    }
  }
  assert(Grid[level][index].u_coeff.size() == M);
  for (int i = 0; i < M; ++i) {
...
void PlotSinglescale(GlobalGrid & Grid, int n, func F) {
  const int levels = Grid.size();
...
void PlotAdaptiveGrid(GlobalGrid & Grid, int n, func F) {
  const int levels = Grid.size();
  int bsp = (int) F(-1.,-1.);
...
void PlotFunction(GlobalGrid & Grid, int n, func F) {
  const int levels = Grid.size();
  int bsp = (int) F(-1.,-1.);
  int points = ((n+2)*(n+3))/2;
  int cells  = (n+1)*(n+1);
...
void PlotSkalierungsfkt(int m, int n) {
  n *= 2; n += 1;
  int points = ((n+2)*(n+3))/2;
  int cells  = (n+1)*(n+1);
  char filename[80];
...
void PlotMultiwavelet(int m, int n) {
  int points = ((n+2)*(n+3))/2;
  int cells  = (n+1)*(n+1);
  char filename[80];
...
void GridAdaptation(GlobalGrid & Grid, double eps, double a) {
  const int levels = Grid.size();
  for (int l = 1; l < levels; ++l) {
    eps /= a;
    for (GridMap::iterator it = (Grid[l]).begin(); it != (Grid[l]).end(); ++it){
...
void initializeGrid(GlobalGrid & Grid, int levels){
  Grid.clear();
  Grid.resize(levels);
  ...
  // Erstes Dreieck
    ...
    ID.flaeche = 0.5*abs((ID.nodes[0].x-ID.nodes[2].x)*(ID.nodes[1].y-ID.nodes[2].y)-(ID.nodes[1].x-ID.nodes[2].x)*(ID.nodes[0].y-ID.nodes[2].y));
    ID.A.resize(3,3);
    ID.A(0,0) = ID.nodes[0].x; ID.A(0,1) = ID.nodes[1].x; ID.A(0,2) = ID.nodes[2].x;
    ID.A(1,0) = ID.nodes[0].y; ID.A(1,1) = ID.nodes[1].y; ID.A(1,2) = ID.nodes[2].y;
    ID.A(2,0) = 1.;            ID.A(2,1) = 1.;            ID.A(2,2) = 1.;
   ...
   for(int i=0 ; i< levels-1 ; i++){
     ...
     for (GridMap::iterator it = (Grid[i]).begin(); it != (Grid[i]).end(); ++it){
       ...
       it->second.childs.resize(4);
         ...
         Insert.A(0,0) = Insert.nodes[0].x; Insert.A(0,1) = Insert.nodes[1].x; Insert.A(0,2) = Insert.nodes[2].x;
         Insert.A(1,0) = Insert.nodes[0].y; Insert.A(1,1) = Insert.nodes[1].y; Insert.A(1,2) = Insert.nodes[2].y;
         Insert.A(2,0) = 1.;                Insert.A(2,1) = 1.;                Insert.A(2,2) = 1.;
...
double ErrorSinglescale(GlobalGrid & Grid, func F, int rule) {
  const int levels = Grid.size();
  int order_num;
  double *wtab;
  double *xytab;
...
double ErrorMultiscale(GlobalGrid & Grid, func F, int rule) {
  const int levels = Grid.size();
  int order_num;
  double *wtab;
  double *xytab;
